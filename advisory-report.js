(function(global){
  ...
  async function build(data){
    ...
    ranked.slice(0,3).forEach(function(item,index){ensure(66);rect(M,ctx.y-52,CONTENT,52,index===0?PALE:SOFT,index===0?BLUE:LINE,1);drawText(String(index+1),M+14,ctx.y-31,{size:17,bold:true,color:index===0?BLUE:MUTED});drawText(item.name,M+48,ctx.y-20,{size:11,bold:true});drawText(AREA_COPY[item.key]||"Persönlichen Handlungsbedarf pr��fen.",M+48,ctx.y-37,{size:7.5,color:MUTED});drawText(item.score+" / 5",PAGE[0]-M-67,ctx.y-24,{size:12,bold:true,color:statusColor(item.status)});drawText(statusLabel(item.status),PAGE[0]-M-86,ctx.y-40,{size:6.5,color:MUTED});ctx.y-=62;});
    ...
    var CALC_TITLES={"wealth":"Sparrechner","wealth-sparen":"Spar- und Zinseszinsrechner","wealth-zins":"Zinsvergleich","wealth-start":"Starten oder warten?","wealth-3a":"Säule 3a","wealth-steuer":"Steuerrechner","wealth-inflation":"Inflationsrechner","wealth-kosten":"Kostenrechner","wealth-ziel":"Sparziel-Rechner","ahv-retirement":"AHV-Rentenschätzung","real-estate-affordability":"Immobilien-Tragbarkeit","health-franchise":"Franchise-Vergleich"};
    Object.keys(modules.calculators||{}).slice(0,8).forEach(function(key){var c=modules.calculators[key];if(!c||!Array.isArray(c.results)||!c.results.length)return;var title=CALC_TITLES[key]||(c.page?safe(c.page).replace(/\.html$/,""):"Rechner: "+key);resultBlock(title,c.source||("Stand "+(c.calculationYear||2026)),c.results.slice(0,3).map(function(value,index){return["Ergebnis "+(index+1),safe(value).slice(0,42),""];}),c.basis?"Grundlage: "+c.basis+".":"");});
