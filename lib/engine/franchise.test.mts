// Runs against the SAME module the app ships (./franchise.ts) — no duplicate copy.
// Execute with: node --test lib/engine/*.test.ts  (Node >= 22 strips TS types natively)
import test from "node:test"
import assert from "node:assert/strict"
import { ageGroupFromBirthYear, compareFranchises, costSharing } from "./franchise.ts"
...
test("comparison uses the exact premium for every offered franchise", () => {
  const result = compareFranchises(
    [
      [300, 420],
      [2500, 300],
    ],
